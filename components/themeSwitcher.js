...
export default function ThemeSwitcher() {
  ...
  //commented code are ment for dark and light mode
  //current fixed to light mode
  ...
  //   }
  // }, [colors]);
  const changeTheme = (theme, idx) => {//removed the arg colormode: boolean
    setIndex(idx);
    // setTheme(theme);
        ...
        {colors.map((item, idx) => (
          <li key={idx} className="flex-none">
            <label
              ...
              aria-label={`Select color: ${item.bg}`}
            >
              <input
                id={item.bg}
                type="radio"
                ...
                onChange={(e) => changeTheme(item.theme, idx)}
              />
              <span
