export default function PostCard({size, imageSrc, post}) {
    ...
    const imageUrl = `${
        post.coverImage.url.startsWith('/') ? process.env.NEXT_PUBLIC_STRAPI_API_URL : ''
      }${post.coverImage.url}`
    
    //const categoryUpper = category.toUpperCase()
    return (
        <div class={"p-4 " + cardSizeClass}>
            <div class="h-full border-2 border-gray-200 border-opacity-60 rounded-lg overflow-hidden">
                <img src={imageUrl} alt="blog" class="lg:h-48 md:h-36 w-full object-cover object-center" />
                <div class="p-6">
                    <h2 class="tracking-widest text-xs title-font font-medium text-gray-400 mb-1">CATEGORY</h2>
                    <h1 class="title-font text-lg font-medium text-gray-900 mb-3">{(post.category.name? post.category.name.toUpperCase(): 'CATEGORY')}
                    </h1>
