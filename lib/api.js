...
async function fetchAPI(query, { variables } = {}) {
  ...
  console.log(cms_url,query, "data of url")
...
export async function getPreviewPostBySlug(slug=null) {
...
export async function getCaseStudies(){
  const data = await fetchAPI(`query casestudies {
  caseStudies {
    data {
      attributes{
        Title
...
// now passing only the first case study which is "single object & not array"// change based on future requirements
return data.caseStudies.data[0].attributes;
...
function transformV4toV3Categories(response) {
  function transformPost(post) {
    const attributes = post.attributes;
    ...
    return {
      ...
      date: attributes.date,
      excerpt: attributes.excerpt,
      coverImage: attributes.coverImage && attributes.coverImage.data
        ? { url: attributes.coverImage.data.attributes.url }
        : null,
      author: attributes.author && attributes.author.data
        ? {
            id: attributes.author.data.id,
            name: attributes.author.data.attributes.name,
            picture: attributes.author.data.attributes.picture && attributes.author.data.attributes.picture.data
              ? { url: attributes.author.data.attributes.picture.data.attributes.url }
              : null
          }
        : null
    };
  ...
  function transformCategory(category) {
    ...
    return {
      ...
      slug: attributes.slug,
      posts: attributes.posts.data.map(transformPost)
    };
  }

  return response.categories.data.map(transformCategory);
}
...
export async function getCategorybySlug(slug=null) {
  ...
  const transformed_data= transformV4toV3Categories(data)
  return transformed_data
}
